...
import { PurchaseDetailEwayValidUpto } from '@/components/purchases/PurchaseDetailEwayValidUpto';
import type { PurchaseDetailEwayValidUptoDisplay } from '@/utils/purchaseDetailEwayValidUpto';
...
function EditPurchaseContent() {
    ...
    const [invoiceFromBe, setInvoiceFromBe] = useState<PurchaseDetailEwayValidUptoDisplay | null>(null);
            ...
            try {
                ...
                if (invoiceId) {
                    ...
                    const mappedRows: PurchaseRow[] = inv.items.map((item: any) => {
                        const prod = fetchedProducts.find((p: any) => p.id === item.product);
                        return {
                            product: prod || { id: item.product, name: item.product_name, barcode: '', price: 0, purchase_price: 0, original_price: 0, image: '' },
                            quantity: Number(item.quantity),
                            purchase_price: Number(item.purchase_price),
                            new_price: Number(prod?.price || 0),
                            new_original_price: Number(prod?.original_price || 0),
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            toast.error(error.response?.data?.error || "Failed to update purchase");
            console.error(error);
        } finally {
    ...
    return (
        <div className="p-8 max-w-[1400px] mx-auto font-sans min-h-screen pb-32">
            ...
            <div className="flex items-center gap-4 mb-8">
                ...
                <div>
                    <h1 className="text-3xl font-black text-gray-900 tracking-tight">Edit Purchase Bill</h1>
                    <p className="text-gray-500 mt-1">Correct or update inward stock details.</p>
                </div>
            </div>
                        ...
                        <div className="flex-1 overflow-x-auto p-4">
                            <table className="w-full text-left">
                                ...
                                <tbody className="divide-y divide-gray-50">
                                    {rows.map((row, idx) => (
                                        <tr key={`${row.product.id}-${idx}`} className="hover:bg-gray-50/50 transition-colors">
                ...
                <div className="space-y-6">
                    <div className="bg-white p-8 rounded-[2.5rem] border border-gray-100 shadow-xl shadow-primary/5 sticky top-8">
                        ...
                        <div className="bg-primary/5 rounded-[2rem] p-6 mb-8 border border-primary/10">
                            ...
                            <div className="mt-4 flex justify-between items-center px-1">
                                <span className="text-xs font-bold text-gray-400">Balance Effect</span>
                        ...
                        <div className="space-y-4">
                            <label className="text-xs font-bold text-gray-400 uppercase tracking-widest flex items-center gap-2 px-1">
                                <Info size={14} /> Edit Reasons / Notes
                            </label>
                            <textarea 
                                value={notes}
                                onChange={(e) => setNotes(e.target.value)}
                                placeholder="Why was this bill updated?"
                        ...
                        >
                            {isSubmitting ? (
                                ...
                                <>
                                    <Save size={20} /> Update Purchase Bill
                                </>
                            )}
                        </button>
                    </div>
                    ...
                    <div className="bg-amber-50 p-6 rounded-[2rem] border border-amber-100 flex gap-4">
                        ...
                        <p className="text-xs font-medium text-amber-700 leading-relaxed">
                            Updating this bill will reverse previous stock changes and apply the new ones. 
                            Distributor balances will also be reconciled automatically.
                        </p>
                    </div>
                </div>
