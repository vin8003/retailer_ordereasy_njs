...
describe("buildLabelPrintDocument", () => {
  ...
  it("uses CODE128 in the print DOM when EAN-13 is preferred but the code is invalid", () => {
    const html = buildLabelPrintDocument(
      sampleContext({
        prefs: {
          sizeId: "50x25",
        ...
        items: [sampleItem({ barcode: "SKU-44" })],
      }),
    );
    expect(html).toContain('data-format="CODE128"');
    expect(html).toContain('data-value="SKU-44"');
  });

  it("pads the last row with empty cells for a 3-column roll", () => {
    const html = buildLabelPrintDocument(
      sampleContext({
        prefs: {
          sizeId: "38x25",
    ...
    expect(html).toContain("@page { size: 114mm 25mm; margin: 0; }");
    expect(html).toContain("grid-template-columns: repeat(3, 38mm)");
