...
export function displayLabelCss(widthMm: number, heightMm: number, isA4: boolean): string {
  // We use slightly larger fonts for display labels since they are viewed from a distance
  return `
    .label {
      width: ${widthMm}mm;
      height: ${heightMm}mm;
...
export function buildDisplayLabelPrintDocument(context: DisplayLabelPrintContext): string {
  ...
  const labels = copies.map(
    (item) => `<div class="label">${renderDisplayLabelInner(item, prefs.barcodeFormat)}</div>`,
  );
  ...
  if (size.isA4 && size.columns && size.rows && size.pageWidthMm && size.pageHeightMm) {
    ...
    // A4 Margins (calculate dynamically based on labels)
...
<head>
  ...
  <style>
    ...
    ${displayLabelCss(size.widthMm, size.heightMm, size.isA4)}
  </style>
</head>
<body>
  ${bodyContent}
