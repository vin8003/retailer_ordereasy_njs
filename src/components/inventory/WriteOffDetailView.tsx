import { WriteOffDetailBatchNumber } from "@/components/inventory/WriteOffDetailBatchNumber";
import type { WriteOffDetailBatchNumberDisplay } from "@/utils/writeOffDetailBatchNumber";

export type WriteOffDetailViewModel = WriteOffDetailBatchNumberDisplay & {
    id?: number;
    product_id?: number;
...
/** Thin write-off detail chrome. Optional batch_number only — no list reason, no created-by invent. */
export function WriteOffDetailView({ writeOff }: { writeOff: WriteOffDetailViewModel }) {
    ...
    return (
        <div className="space-y-4">
            ...
            <WriteOffDetailBatchNumber writeOff={writeOff} />
        </div>
    );
}
