...
import type { WriteOffDetailInspectorNameDisplay } from "@/utils/writeOffDetailInspectorName";

export type WriteOffDetailViewModel = WriteOffDetailInspectorNameDisplay & {
    id?: number;
    product_id?: number;
...
/** Thin write-off detail chrome. Optional inspector_name only — no list reason, no batch invent. */
export function WriteOffDetailView({ writeOff }: { writeOff: WriteOffDetailViewModel }) {
    ...
    return (
        <div className="space-y-4">
            ...
            <WriteOffDetailInspectorName writeOff={writeOff} />
        </div>
    );
}
